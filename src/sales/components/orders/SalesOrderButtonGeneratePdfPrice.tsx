...
export const SalesOrderButtonGeneratePdfPrice: FC<Props> = ({
  className,
  tooltip
}) => {
  ...
  if (!authContext) 
    throw new Error("SalesOrderButtonGeneratePdfPrice: AuthContext must be used within an AuthProvider");
  ...
  const generatePDF = async () => {
    try {
      ...
      // * add customer data
      const customerData = [
        ['RUT:'       , `${obj.customerIdDoc?.toUpperCase()}`],
        ['Nombre:'    , `${formatCapitalized(obj.customerName)}`],
        ['Email:'     , `${obj.customerEmail?.toLowerCase()}`],
        ['Dirección:' , `${formatCapitalized(obj.customerAddress)}`],
      ];
      ...
      const productList: string[][] = obj.productList.reduce((acc: string[][], value) => {
      ...
      if (availableSpace < alturaTablaTotales) {
       ...
       // * add totals data
      const subTotal  = formatNumber(obj.subTotal ?? 0);
      const iva       = formatNumber(obj.iva ?? 0);
      const total     = formatNumber(obj.total ?? 0);
      ...
      finalY += 40;
      doc.setFontSize(12);
      {obj.comment ? doc.text("Información General:", 10, finalY, { align: "left" }) : ""};
