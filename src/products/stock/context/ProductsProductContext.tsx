...
import { FormProductsProductSearchInterface, FormProductsProductInterface, FormProductsProductElementInterface, FormProductsProductErrorInterface, ProductsProductInterface, ProductsProductElementInterface, ProductsProductUnitInterface } from '../interfaces';
...
interface ProductsProductContextType {
  ...
  searchProducts: (withMovements: boolean, nameCode?: string, typeList?: ProductTypeEnum[], productTypeId?: string) => Promise<ProductsProductInterface[]>;
  ...
  // formProductElementList : FormProductsProductElementInterface;
...
export const ProductsProductProvider: FC<Props> = ({ children }) => {
  ...
  useEffect(() => {
    setForm({ 
      ...form, 
      cost: calculateProductCost(formProductElementList), 
  ...
  const searchProducts = (withMovements: boolean, nameCode?: string, productTypeList?: ProductTypeEnum[], productCategoryId?: string): Promise<ProductsProductInterface[]> => {
    
    return fetchProducts({ variables: { nameCode, productTypeList, productCategoryId, withMovements } })
    .then( (response) => {
  ...
  const calculateProductCost = (formProductElementList: FormProductsProductElementInterface[] = []): number => {
    ...
    for (const formProductElement of formProductElementList) {
      ...
      if(formElement.type === ProductTypeEnum.PC || formElement.type === ProductTypeEnum.PCC) {
        cost += formProductElement.qty * calculateProductCost(formElement.elementList);
      } else {
  ...
  const calculateProductType = (formElementList: FormProductsProductElementInterface[] = []): ProductTypeEnum => {
