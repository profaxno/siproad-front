...
interface Props {
  value: FormInventoryProductElementInterface;
  selectedRow: string;
  onNotifyClick: (formProductElement: FormInventoryProductElementInterface) => void;
}

export const InventoryProductElementTableItem: FC<Props> = ({
  value,
  selectedRow,
  onNotifyClick,
}) => {
  ...
  if (!context) 
    throw new Error("InventoryProductElementTableItem: InventoryProductContext must be used within an InventoryProductProvider");
  
  const { form, updateTableProductElement } = context;
  const [formProductElement, setFormProductElement] = useState<FormInventoryProductElementInterface>({ ...value });
