...
import { FormInventoryProductElementInterface, FormInventoryProductInterface, InventoryProductInterface } from '../interfaces';
...
const initProductElement: FormInventoryProductElementInterface = {
  key     : '',
  element: initForm,
  // id      : '',
  qty     : 1,
...
export const InventoryProductElementSearch: FC = () => {
  
  // * hooks
  const context = useContext(InventoryProductContext);
  if (!context) 
    throw new Error("InventoryProductElementSearch: InventoryProductContext must be used within an InventoryProductProvider");
  ...
  // if (!productContext) 
  //   throw new Error("InventoryProductElementSearch: ProductsElementContext must be used within an ProductsElementProvider");

  // const { searchElements } = productContext;
  
  const [formProductElementSearch, setFormProductElementSearch] = useState<FormInventoryProductElementInterface>(initProductElement);
  ...
  const updateForm = (obj: InventoryProductInterface) => {
    ...
    const formProductElement: FormInventoryProductElementInterface ={
      key     : uuidv4(),
      element: form,
      // id      : product.id,
  ...
  const search = (value: string): Promise<InventoryProductInterface[]> => {
    ...
    .then( (productList: InventoryProductInterface[]) => productList.filter(value => value.id != form.id) )
    .catch( (error: any) => {
  ...
  const handleButtonAdd = () => {
    ...
    // const orderElementAux: FormInventoryProductElementInterface = {
